...
class Intro(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        # Welche Spielart (Spiel 1 bis 5) kommt wann vor
        session.Spielarten_Folge = [1, 4, 2, 5, 3, 2, 5, 4, 3, 1]

        # BERATER VERDIENT JETZT ANTEILIG - Reihenfolge obsolet
...
class WarteAufDieOpfer(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        ...
        if (group.EndgueltigeEntscheidung is not None):
            ...
            # Berater wird nur am Ende mit z.B. 80% ausgezahlt, Inkrement hier - NULL
...
class AuszahlungUmfrage(Page):
    ...
    def vars_for_template(player: Player):
        ...
        # BERATER BEKOMMT jetzt z.B. 80% des Entscheiders. Inkrement war 0, berechnet wird hier
        ...
        anteil = Constants.berater_anteil_in_prozent/100
        beraterAuszahlung = session.EntscheiderAuszahlungPunkte*anteil
        session.BeraterAuszahlungPunkte = int(round(beraterAuszahlung, 0))
        group.BeraterAuszahlungPunkte = session.BeraterAuszahlungPunkte
        group.BeraterAuszahlungDKK = round(session.BeraterAuszahlungPunkte / Constants.waehrungsFaktorDKK,0)
