...
class QuizEntscheider(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        # Nur für den Entscheider und nur in der ersten Runde
        return (player.participant.zugeordneteRole == Constants.entscheider_role) and (player.round_number == 1)
...
# Auswahl des Entscheiders. Opfer wartet
...
class WarteAufDieOpfer(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        ...
        # Berater wird durch den Computer ersetzt
...
class SeiteFragenAnDenEntscheider(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        # Nur für den Entscheider und nur in der letzten Runde
...
page_sequence = [Intro, RoleEntscheider, RoleOpfer, QuizEntscheider, SeiteFuerDenEntscheider, WarteAufDenEntscheider, SeiteFuerDieOpfer, WarteAufDieOpfer, SeiteFragenAnDenEntscheider, AuszahlungUmfrage, ErgebnisComputerCouncil]