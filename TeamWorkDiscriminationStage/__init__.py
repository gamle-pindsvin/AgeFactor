...
class Constants(BaseConstants):
    ...
    maximaleAnzahlProGruppeT1 = 2
    maximaleAnzahlProGruppeT2 = 2
    maximaleAnzahlProGruppeT3 = 2
    maximaleAnzahlProGruppeT4 = 2
...
class Player(BasePlayer):
    ...
    QuizKodieren1 = models.StringField(choices=[['a', 'a'], ['b', 'b'], [
                                       'c', 'c']], widget=widgets.RadioSelect, label='')
    QuizKodieren2 = models.StringField(choices=[['a', 'a'], ['b', 'b'], [
                                       'c', 'c']], widget=widgets.RadioSelect, label='')
    QuizBezahlung = models.StringField(choices=[['a', 'a'], ['b', 'b'], [
                                       'c', 'c']], widget=widgets.RadioSelect, label='')
    ...
    Education = models.StringField(choices=[['1', 'No formal qualifications'], ['2', 'GCSEs or equivalent (e.g., O-Levels)'], ['3', 'A-Levels or equivalent (e.g., high school diploma)'], ['4', 'Vocational qualification (e.g., NVQ, BTEC)'], [
                                   '5', 'Undergraduate degree (e.g., BA, BSc)'], ['6', 'Postgraduate degree (e.g., MA, MSc, PhD)'], ['0', "Other "]], widget=widgets.RadioSelect, label='What is the highest level of education you have completed? ')
    GeburtsJahr = models.IntegerField(
        min=1930, max=2010, label='In which year were you born?')
    Gender = models.StringField(choices=[['F', 'Female'], ['M', 'Male'], ['D', 'Not listed'], [
                                'N', 'Prefer not to answer']], widget=widgets.RadioSelect, label='What is your gender?')
    ...
    Akzeptiert_bedingungen = models.BooleanField(
        label="", widget=widgets.CheckboxInput, blank=True)
...
class Intro(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        while weiterRolleSuchen:
            ...
            gruppen_attribut = f'anzahlSpielerInDerGruppe{zufallszahl}'
            if getattr(gruppe, gruppen_attribut) >= 1:
                setattr(gruppe, gruppen_attribut, getattr(gruppe, gruppen_attribut) - 1)

                player.zugeordneteRole = zufallszahl
                player.participant.zugeordneteRole = zufallszahl
                weiterRolleSuchen = False


        print("Rolle: ", player.zugeordneteRole)
...
class Quiz(Page):
    ...
    def error_message(player, values):
        ...
        quizKorrekt = ((values['QuizKodieren1'] == 'b') and (
            values['QuizKodieren2'] == 'c') and (values['QuizBezahlung'] == 'b'))
    ...
    def vars_for_template(player: Player):
        ...
        antwortB = Constants.quizAngenommeneAnzahlAntowrten* \
            Constants.bezahlungProAntwort + Constants.festerAnteilderBezahlung
        antwortC = Constants.quizAngenommeneAnzahlAntowrten + \
            Constants.festerAnteilderBezahlung
        return {
            'AntwortA': antwortA,
            'AntwortB': antwortB,
            'AntwortC': antwortC
...
class AuszahlungUmfrage(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        if not player.HatSichQualifiziert:
            ...
            player.participant.AnzahlRichtigerAntworten = 0
            player.participant.VerdientePunkte = Constants.festerAnteilderBezahlung
            player.participant.payoff = player.participant.VerdientePunkte * \
                Constants.waehrungsFaktor
